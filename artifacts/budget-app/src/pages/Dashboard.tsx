import { useGetDashboardSummary, useGetExpenses } from "@workspace/api-client-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader 
        title="דשבורד תקציבי" 
        description="מבט על של המצב הפיננסי שלך לחודש הנוכחי" 
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="shadow-sm border-border/50">
          <CardHeader className="border-b border-border/50 bg-muted/30 pb-4">
            <CardTitle className="text-lg font-bold">התפלגות הוצאות</CardTitle>
          </CardHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <StatCard
            title="סה״כ חובות"
            value={formatILS(summary?.totalDebts)}
            icon={<CreditCard className="w-5 h-5 text-amber-600" />}
          />
          <StatCard
            title="סה״כ חסכונות"
            value={formatILS(summary?.totalSavings)}
